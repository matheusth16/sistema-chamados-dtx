...
- `scripts/migrations/` — migrações one-shot de dados no Firestore (`migrar_*.py`, `atualizar_*.py`, `adicionar_setor_ti.py`, `_migration_utils.py`)
- `scripts/qa/` — playbooks de QA manual e testes de carga (`executar_qa_*.py`, e-mails visuais UAT, `k6/`)
- `scripts/seed/` — inicialização de dados (`criar_usuario.py`, `init_categorias.py`, `seed_dados_demo_onboarding.py`)
- Raiz de `scripts/` — utilitários isolados (chaves, diagnóstico, gates de qualidade, reset/limpeza automatizados)
...
python scripts/qa/executar_qa_manual_cwi.py --json > docs/evidencias/qa_manual_cwi_resultado.json
```
...
```bash
python scripts/migrations/migrar_pii_criptografia.py              # dry-run: lista o que seria migrado
ENCRYPT_PII_AT_REST=true ENCRYPTION_KEY=<chave> \
  python scripts/migrations/migrar_pii_criptografia.py --apply    # criptografa e persiste
```
...
```bash
python scripts/migrations/migrar_setores_catalogo.py             # dry-run (seguro, apenas lista)
python scripts/migrations/migrar_setores_catalogo.py --apply     # grava de verdade
```
...
```bash
python scripts/migrations/migrar_gates_subetapas.py              # dry-run (seguro, apenas lista)
python scripts/migrations/migrar_gates_subetapas.py --apply      # grava de verdade

python scripts/migrations/migrar_grupos_rl.py                    # dry-run (seguro, apenas lista)
python scripts/migrations/migrar_grupos_rl.py --apply            # grava de verdade

python scripts/migrations/migrar_campo_senha.py
```
...
```bash
python scripts/migrations/migrar_usuarios_ativo.py           # dry-run: lista o que seria atualizado
python scripts/migrations/migrar_usuarios_ativo.py --apply   # grava ativo=true nos docs sem o campo
```
...
python scripts/migrations/migrar_participantes.py

# Aplicar — gravar participantes[] nos chamados sem o campo
python scripts/migrations/migrar_participantes.py --apply
```
...
python scripts/qa/disparar_testes_emails_visual_uat.py
```
...
python scripts/migrations/atualizar_setores_from_print.py            # dry-run
python scripts/migrations/atualizar_setores_from_print.py --apply    # grava
```
