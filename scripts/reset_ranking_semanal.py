...
def resetar_ranking_semanal():
    ...
    try:
        ...
        for doc in docs:
            ...
            if gamification.get("exp_semanal", 0) > 0:
                ...
                if batch_count >= 500:
                    ...
                    print(
                        colored(
                            f"Batch comitado: {batch_count} usuários (Total até agora: {usuarios_atualizados})",
                            "yellow",
                        )
                    )
                    batch = db.batch()
        ...
        print(
            colored(
                f"Sucesso! {usuarios_atualizados} usuários tiveram o EXP Semanal resetado.", "green"
            )
        )
        return True
