...
| **F-85** | **Autoatendimento LGPD parcial — `/meus-dados` cobria só direito de acesso; sem exportação/portabilidade self-service, sem exclusão self-service, sem política automatizada de retenção (dados ficavam indefinidamente por padrão)** | `app/routes/auth.py`, `app/services/lgpd_self_service.py` (novo) | **Resolvido** 2026-07-22 — achado por auditoria com skills `/aas`; export JSON via `GET /meus-dados/exportar` (rate-limited); solicitação de exclusão via `POST /meus-dados/solicitar-exclusao` (não executa na hora — registra pedido em `solicitacoes_lgpd`, admin revisa e executa via fluxos existentes de desativar+anonimizar); badge de aviso em `/admin/usuarios` para pedidos pendentes; 22 testes novos | — |
...
| F-30 | `SETOR_PARA_AREA` com apenas 2 mapeamentos hardcoded — novo setor requer deploy | `utils_areas.py:7-10` | **Resolvido** 2026-06-19, fonte de verdade migrada de Firestore pra Postgres no Marco 12 (2026-08-04) — `utils_areas.setor_para_area()` lê `config_setor_area` (Postgres); fallback estático + cache TTL 5 min + `invalidar_cache_setor_area()`. Script `migrar_setor_area.py` (gravava no Firestore legado, sem efeito desde o Marco 12) removido em 2026-08-14 — dado já semeado no Postgres via `migrate_firestore_to_postgres.py::_load_config_setor_area` | Onda C w3 |
...
| **F-84** | **`.stream()` sem `.limit()` em 7 métodos (`Usuario.get_all()`, 6 métodos de `models_categorias.py`) — viola a regra "sem paginação em coleções grandes" do CLAUDE.md** | `app/models_usuario.py:450`, `app/models_categorias.py` (6 locais) | **Resolvido** 2026-07-22 — achado por auditoria com skills `/aas`; teto de segurança via `.limit()` (`MAX_USUARIOS_GET_ALL=2000`, `MAX_CATEGORIAS=1000`) + warning log quando atingido; 20 testes novos/ajustados (TDD) | — |
| **F-86** | **`cd-build-image.yml` publica imagem no GHCR sem scan de vulnerabilidade; nenhum workflow com scan de segredo dedicado (só `detect-private-key` do pre-commit)** | `.github/workflows/cd-build-image.yml`, `.github/workflows/ci.yml` | **Resolvido** 2026-07-22 — achado por auditoria com skills `/aas`; Trivy (scan de imagem, SARIF) e gitleaks (scan de segredo) adicionados, ambos não-bloqueantes (`continue-on-error`) reportando via artifact — endurecer para bloqueante depois de um período triando achados | — |
