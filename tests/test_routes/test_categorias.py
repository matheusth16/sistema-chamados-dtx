...
def test_admin_categorias_com_admin_retorna_200(client_logado_admin):
    ...
    with patch('app.routes.categorias.CategoriaSetor.get_all', return_value=[]), \
    ...
    assert b'categorias' in r.data.lower() or b'setor' in r.data.lower() or b'gate' in r.data.lower()
