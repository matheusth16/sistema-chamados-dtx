...
def test_obter_contexto_admin_retorna_dict_com_chaves_esperadas():
    ...
    with patch('app.services.dashboard_service.get_static_cached') as mock_cache:
        ...
        with patch('app.services.dashboard_service.db') as mock_db:
            ...
            with patch('app.services.dashboard_service.aplicar_filtros_dashboard_com_paginacao') as mock_filtros:
...
def test_filtrar_chamados_por_permissao_supervisor_filtra_por_area():
    ...
    with patch('app.services.dashboard_service.Usuario.get_by_id', return_value=None):
        with patch('app.services.dashboard_service.usuario_pode_ver_chamado_otimizado') as mock_perm:
